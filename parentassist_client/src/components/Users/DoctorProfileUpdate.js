...
function DoctorProfileUpdate() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('http://localhost:9000/getDoctorData', { params: { user_child_id }});
    ...
    const handleGeneralData = async (e) => {
        ...
        try {
            const response = await axios.put('http://localhost:9000/Doctorprofileupdate', { name, specialization,hospital, phone, user_child_id });
    ...
    const handleUserPass = async (e) => {
        ...
        try {
            const response = await axios.put('http://localhost:9000/ProfilePassUpdate', { password, user_child_id });
            if (response.status === 200) {
